...
type MarkdownSearcher interface {
	SearchMarkdown(ctx context.Context, query string, providers []string, deepSearch *bool, indexWrite *bool) (string, error)
}
...
func (s *Search) Handle(w http.ResponseWriter, r *http.Request) {
	...
	md, err := s.Orch.SearchMarkdown(r.Context(), req.Query, req.Providers, req.DeepSearch, req.IndexWrite)
